from IHSetYates09 import cal_Yates09_2
import xarray as xr
import time as t_
...
path = r"C:\Users\freitasl\Documents\ReposGIT\metrics_for_y09_model\data\Angourie_IH_SET.nc"
...
dataset.attrs['Yates09'] = config_json
...
par_values = model.par_values
par_names = model.par_names

for value, name in zip(par_values, par_names):
    print(f'{name}: {value}')

full_run = model.full_run

print('Time elapsed: %.2f seconds' % (t_.time() - starting_time))

fig, ax = plt.subplots(1, 1, figsize=(12, 2))

ax.plot(model.time_obs, model.Obs, 'k-s', label='Observations', markersize=1, linewidth=0.3)
ax.plot(model.time, full_run, 'r-', label='Model', linewidth=1.5)
